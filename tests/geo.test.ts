...
import { resolveMapCoords, STATE_CENTROIDS, stateCentroid } from '@/lib/data/geo';
...
describe('resolveMapCoords', () => {
  it('returns exact coords with approximate=false when both lat and lng are set', () => {
    ...
    expect(result).toEqual({ lat: 10.5, lng: -66.9, approximate: false });
  });
  ...
  it('uses provided coords even when a valid estado is given', () => {
    ...
    expect(result).toEqual({ lat: 1.0, lng: -60.0, approximate: false });
  });
});
